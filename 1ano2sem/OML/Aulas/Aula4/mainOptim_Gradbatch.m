...
%--
E=[];  % para guardar os valores da função custo MSE

while ( norm(gradiente(wk,Xt,Yt))> tol && k<=maxit)  % -- no estocástico e mini-batch usar apenas número de iterações!!
    
    %calcular o gradiente batch no ponto wk, i.é., usa todo o dataset de treino
    gradk=gradiente(wk,Xt,Yt);  %calcula o gradiente completo no ponto wk   
    %calcular a direção de procura
    sk=-gradk; 
    
    %%%%
    Ek=funE(wk,Xt,Yt);  %calcular a função custo MSE no ponto wk
    %%%%%%%%%%%%%%%%%%%%%%%%%% etak
    ...
    %----  para fazer o gráfico dos valores Ek da função custo MSE 
    E=[E,Ek];
