...
%Dataset treino - 80% do Data -- selecionado aleatoriamente
...
while (k<=maxit)  % -- no estocástico e mini-batch usar apenas número de iterações!!
    ...
    %escolher aleatoriamente um indice no intervalo de  1:nt
    i=randi(nt); 

    %calcular o gradiente estocástico no ponto wk, usando o índice i do dataset de treino 
    gradk=gradiente(wk,Xt(i),Yt(i));
    %calcular a direção de procura
    sk=-gradk; 
    
    %%%%
    Ek=funE(wk,Xt(i),Yt(i))  %calcular a função MSE no ponto wk, usando o índice i 
    %%%%%%%%%%%%%%%%%%%%%%%%%% etak
    %calcular etak, a taxa de aprendizagem/comprimento do passo
     
    if paramP==1  %alg. da condição Armijo com backtracking
      etak = backtrackingArmijo(wk,Ek,gradk,sk, Xt(i),Yt(i));  % re_adapatada 
    end
