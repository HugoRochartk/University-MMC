...
irreg_lattice <- read.table("areal data/CentPoint.txt", header = TRUE)
...
plot_1_6 <- read.table("areal data/Plot_1_6.txt", header = TRUE)
plot_22_7 <- read.table("areal data/Plot_22_7.txt", header = TRUE)
...
nb
W <- nb2listw(nb, style = "W") #atenção ao argumento "style"
?moran.test
moran.test(plot_1_6$height, W, alternative = "two.sided")

###########
# EXERCICIO: Analisar se há correlação espacial entre as alturas das árvores na região 2. E os diametros são correlacionados?
###########
nb <- tri2nb(as.matrix(plot_22_7[, 1:2]))
nb
W <- nb2listw(nb, style = "W") #atenção ao argumento "style"
...
summary(model_SAR)
names(model_SAR)
model_SAR1 <- spautolm(height ~ diameter, data = plot_1_6, W, 
              method = "eigen", family="SAR")
summary(model_SAR1)
